...
const ManageRequest = () => {
	const [requests, setRequests] = useState<
		{ animal: IAnimal; requests: IRequest[] }[]
	>([]);
		...
		async (request: IRequest) => {
			...
			try {
				...
				const response = await fetch(
					`${baseURL}/requests/associations/${request.id}`,
					{
						method: "PATCH",
						headers: {
			...
			{requests.map((animalGroup) => (
				<section className="request" key={animalGroup.animal.id}>
					...
					<Toast
						...
						delay={5000}
						autohide
						className={`mb-2 text-white ${toastData?.color ? `toast-${toastData.color}` : ""}`}
					>
						...
						<tbody>
							{animalGroup.requests.map((request) => (
								<tr
									key={request.id}
									className={
										request.status === "Terminée" ? "table-active" : ""
									}
								>
									<td>{request.id}</td>
									<td>
									...
									{editingItem === request.id ? (
										...
										<td className="edit">
											<Icon
												...
												alt={"icône modifier"}
												onClick={() =>
													handleClickOnEditStatus(request.id, request.status)
												}
											/>
										</td>
									)}
