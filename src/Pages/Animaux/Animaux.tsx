...
const Animaux = () => {
	...
	const [animalsFilterCount, setAnimalsFilterCount] = useState<number | null>(
		null,
	);
		...
		try {
			...
			if (!form) {
				response = await fetch(
					`${import.meta.env.VITE_API_URL}/animals?page=${page}`,
				);
			} else {
				response = await fetch(
				...
				<div className="container-md">
					<section className="intro">
						...
						<p className="intro__text__animals">
							Dans notre application, vous pouvez facilement rechercher des
							animaux en fonction de plusieurs critères. Que vous soyez à la
							recherche d'un compagnon spécifique ou que vous souhaitiez
							simplement explorer les options disponibles, notre fonctionnalité
							de recherche vous permet de filtrer les résultats par type
							d'animal, localisation, association, genre, âge et taille. Que
							vous souhaitiez un petit chien dynamique ou un chat âgé et calme,
							PetFoster Connect vous aide à trouver l'animal qui correspond
					...
					<h2 className="animals__number__results">
						{form
							? `${animalsFilterCount} Résultats`
							: `${animals.length} Résultats`}
					</h2>
